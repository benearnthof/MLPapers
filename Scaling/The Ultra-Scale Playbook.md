...
GPU architecture (for NVIDIA H100): 132 Streaming Multiprocessors (SMs) with 128 cores per SM, summing up to a total of 16896 cores in total, each able to process multiple threads simultaneously.
The GPU memory hierarchy is organized in multiple layers as well. Each thread has access to *Registers* that are private to the thread during execution. *Shared memory* and *L1 Cache* are shared between threads running on a single SM. *L2 Cache* is shared by all SMs and finally we arrive at the *global memory* (VRAM/HBM/SDRAM), which is the largest memory on the GPU, but also the slowest to access and query. 

Kernels (low level programs) are typically written in CUDA or generated with torch.compile or triton, and then compiled to PTX (Parallel Thread Execution), the low-level assembly used by NVIDIA GPUs. Threads are launched in *Warps*, Groups of 32 Threads that are synchronized to execute instructions simultaneously but on different parts of the input data. 
Warps are grouped into *blocks* of a more flexible size, with each block assigned to a single SM. 
```bash
export TORCH_LOGS="output_code"
```
This environment variable allows us to inspect the kernels generated by @torch.compile.

### Memory Coalescing
Because memory accesses to HBM are *synchronous* and happen in *bursts*, meaning in contiguous chunks of size 32, it is most efficient to write kernels in a way that *coalesce* or combine requests to HBM into one large, efficient access request (a DRAM burst), rather than handling each access individually. 

Really interesting blog post on optimizing CUDA Matmul for cuBLAS-like performance
...
GEMM example: Instead of launching a thread for each coordinate in the output matrix we instead load memory in blocks that have a contiguous access pattern and fit into a single burst. This increases throughput by a factor of 10.

This can further be enhanced with Tiling & Thread Coarsening, as well as the minimization of Control Divergence across threads. 
